...
public class Paths {
	 ...
	 * does not exist. Also will make hostile nodes extremely unappealing once we have visited two.
	...
	public static List<Node> shortestPath(Node start, Node end) {
		...
		while (F.size() != 0) {
			...
			int fDist= fData.distance;
			int fhealth = fData.health;

			for (Edge e : f.getExits()) {// for each neighbor w of f
				Node w= e.getOther(f);
				int newWhealth = fhealth;
				...
				if (newWhealth > 2){
					newWdist = Integer.MAX_VALUE;
				}

				SFdata wData= map.get(w);

				if (wData == null) { //if w not in S or F
					map.put(w, new SFdata(newWdist, f, newWhealth));
					F.add(w, newWdist);
				}
	...
	private static class SFdata {
		...
		private SFdata(int d, Node p, int h) {
			...
			health = h; // Number of hostile planets on the path
		}
